...
scp -o ConnectTimeout=10 "$PSScriptRoot\talk_module\web_app.py" "${sshHost}:/home/lab/G1-TalkModule-OpenAiAPI/talk_module/web_app.py"
if ($LASTEXITCODE -ne 0) { Write-Host "Copia fallita" -ForegroundColor Red; exit 1 }

Write-Host "2. Riavvio server sull'AI Accelerator..." -ForegroundColor Yellow
$restartCmd = 'cd /home/lab/G1-TalkModule-OpenAiAPI && pkill -f talk_module.web_app 2>/dev/null; sleep 2; nohup .venv/bin/python3 -m talk_module.web_app --host 0.0.0.0 --port 8081 --no-audio-check > /tmp/talk.log 2>&1 & sleep 3; tail -5 /tmp/talk.log'
